...
router.get('/', (req, res) => {
    ...
    dbClient.connect().then(() => {
        dbClient.db("test").collection("products").find({}).toArray().then((products) => {
...
router.post('/', (req, res) => {
    ...
    dbClient.connect().then(() => {
        ...
        dbClient.db("test").collection("products").insertOne(new_product).then((r) => {
...
router.put('/:name', (req, res) => {
    const dbClient = new MongoClient(url);
    dbClient.connect().then(() => {
        dbClient.db("test").collection("products").updateMany({name: req.params.name}, {$set: {price: req.body.price}},
            (err, result) => { 
...
router.delete('/:name', (req, res) => {
    const dbClient = new MongoClient(url);
    dbClient.connect().then(() => {
        dbClient.db("test").collection("products").findOneAndDelete({name: req.params.name}).then((doc,err) =>{
